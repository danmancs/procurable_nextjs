...
const Header = () => {
  ...
  return (
    <header className={cn("", backgroundStyle)}>
      <div className={cn(`max-w-7xl mx-auto py-16 px-14 sm:px-6 lg:px-8 flex flex-col ${flexLayout} items-center`)}>
        {/* Conditionally render image */}
        {hydrated && imageSrc && (
          <div className={cn(`w-full lg:w-1/2 flex justify-center ${imageAlignmentClass} mb-10 lg:mb-0`)}>
            <Image src={imageSrc} alt="Relevant Image" width={400} height={400} className={cn(`rounded-lg`)} />
          </div>
        )}

        {/* Text content */}
        <div className={cn(`w-full lg:w-1/2 text-center lg:text-left`)}>
          <h1 className={cn(`font-sans font-bold text-3xl md:text-4xl lg:text-6xl text-center leading-snug`)}>
            {title}
          </h1>
          <p className={cn(`mt-10 text-xl lg:text-3xl text-center lg:text-centre`)}>
            {subtitle}
            <br />
            <br />
          </p>
      ...
      {hydrated && router.pathname === `/` && (
        <div className={cn(`flex justify-center w-full`)}>
          <div className={cn(`mt-4 w-full`)}>
            <p className={cn(`font-mono uppercase text-center font-medium text-sm text-gray-600`)}>
              Thanks for stopping by
            </p>
            <div className={cn(`flex items-center justify-center mx-auto flex-wrap`)}>
              <div className={cn(`m-12 mb-8`)}>
